      ...
      <div className="max-w-[1400px] mx-auto space-y-10 relative z-10">
          ...
          {trainersData.map((trainer) => (
            ...
            >
              <div className="space-y-4">
                ...
                {/* Hero Portrait */}
                <div className="relative h-64 md:h-72 overflow-hidden bg-stone-100 border-b border-amber-200/80">
                  <img
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="bg-white/90 border border-amber-200 text-amber-800 text-[10px] font-mono font-bold uppercase tracking-widest px-3.5 py-1 rounded-full backdrop-blur-md shadow-sm">
                      {trainer.experience} Exp
                    </span>
                  </div>

                  <div className="absolute bottom-4 left-5 right-5 space-y-0.5">
                    <h2 className="text-xl md:text-2xl font-serif font-bold text-white uppercase">
                      {trainer.name}
                    </h2>
                    <span className="text-xs font-mono text-amber-200 font-medium block">
                      {trainer.title}
                    </span>
                  </div>
                </div>
                ...
                <div className="px-5 md:px-6 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {trainer.specialties.map((spec, idx) => (
                      <span
                        key={idx}
                        className="text-[11px] font-mono bg-stone-50 border border-amber-200/80 text-stone-700 px-2.5 py-0.5 rounded-lg shadow-sm"
                      >
                        {spec}
                      </span>
                    ))}
                  ...
                  {/* Quote */}
                  <div className="relative bg-amber-50/60 border border-amber-200/80 rounded-2xl p-3.5">
                    <Quote className="w-4 h-4 text-amber-400/50 absolute top-3 right-3" />
                    <p className="text-xs text-stone-900 italic font-serif leading-relaxed pr-5">
                      "{trainer.quote}"
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 py-3 border-t border-b border-amber-100 font-mono text-center">
                    <div>
                      <span className="block text-base font-bold text-amber-700">{trainer.stats.clientTransformations}</span>
                      <span className="text-[9px] uppercase tracking-wider text-stone-500 block">Transformations</span>
                    </div>
                    <div>
                      <span className="block text-base font-bold text-stone-900">{trainer.stats.proAthletesCoached}</span>
                      <span className="text-[9px] uppercase tracking-wider text-stone-500 block">Pro Athletes</span>
                    </div>
                    <div>
                      <span className="block text-base font-bold text-amber-700">{trainer.stats.satisfactionRate}</span>
                      <span className="text-[9px] uppercase tracking-wider text-stone-500 block">Satisfaction</span>
                    </div>
                  </div>
        ...
        {bookingTrainer && (
          <div className="fixed inset-0 z-50 bg-stone-950/50 backdrop-blur-md flex items-center justify-center p-4 font-sans">
            <div className="bg-white/95 backdrop-blur-sm border border-amber-200/80 rounded-3xl max-w-lg w-full p-6 space-y-5 relative shadow-xl">
              ...
              {formSubmitted ? (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-center space-y-2">
                  ...
                  <h4 className="text-base font-serif font-bold text-stone-900">REQUEST RECEIVED</h4>
                  <p className="text-xs text-stone-600 font-normal">
                    Our team will contact you within 2 hours.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleBookSubmit} className="space-y-3.5 text-xs font-mono">
                  <div>
                    <label className="block text-stone-500 uppercase tracking-wider text-[10px] mb-1 font-bold">
                      Full Name *
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-stone-50/80 border border-amber-200 rounded-xl px-3.5 py-2.5 text-stone-900 placeholder-stone-400 focus:outline-none focus:border-amber-600 focus:bg-white transition-colors"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select className="w-full bg-stone-50/80 border border-amber-200 rounded-xl px-3.5 py-2.5 text-stone-900 focus:outline-none focus:border-amber-600 focus:bg-white transition-colors">
                  ...
                  <button
                    type="submit"
                    className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-700 hover:to-yellow-700 text-white font-bold text-xs uppercase tracking-[0.2em] py-3.5 rounded-xl transition-all shadow-md shadow-amber-600/25 cursor-pointer"
                  >
