...
export default function Equipment() {
  ...
  const categories = [
    { id: 'all', label: 'All Apparatus', icon: <Layers className="w-4 h-4 lg:w-5 lg:h-5" /> },
    { id: 'freeweights', label: 'Free Weights & Racks', icon: <Dumbbell className="w-4 h-4 lg:w-5 lg:h-5" /> },
    { id: 'cables', label: 'Cable & Pulley Systems', icon: <Zap className="w-4 h-4 lg:w-5 lg:h-5" /> },
    { id: 'plate-loaded', label: 'Plate-Loaded & Leverage', icon: <Sliders className="w-4 h-4 lg:w-5 lg:h-5" /> },
    { id: 'cardio', label: 'Functional & Endurance', icon: <Activity className="w-4 h-4 lg:w-5 lg:h-5" /> },
  ];
    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto space-y-10 relative z-10">
        ...
        <div className="text-center max-w-3xl lg:max-w-4xl mx-auto space-y-3 font-sans">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 lg:gap-2.5 pt-3">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => handleCategoryClick(cat.id)}
                className={`flex items-center gap-2 text-xs lg:text-sm font-bold uppercase tracking-wider px-4 py-2.5 lg:px-5 lg:py-3 rounded-xl transition-all duration-300 border cursor-pointer ${
        ...
        <div ref={topRef} className="relative bg-white/95 backdrop-blur-sm border border-amber-200/80 rounded-3xl p-6 md:p-8 lg:p-10 overflow-hidden shadow-sm font-sans scroll-mt-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-7 relative group rounded-2xl overflow-hidden border border-amber-200/80 bg-stone-900 aspect-video">
              ...
              {/* Overlay Badges */}
              <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-white/90 border border-amber-200 backdrop-blur-md px-3.5 py-1 lg:px-4 lg:py-1.5 rounded-full text-[10px] lg:text-xs font-mono uppercase tracking-widest text-amber-800 font-bold shadow-sm">
                <Rotate3d className="w-3.5 h-3.5 lg:w-4 lg:h-4 animate-spin text-amber-600" style={{ animationDuration: '8s' }} />
                <span>3D Render Loop Active</span>
              </div>

              <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs lg:text-sm font-mono text-stone-200">
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="space-y-2 lg:space-y-2.5 font-mono text-xs lg:text-sm text-stone-700 pt-1">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 lg:w-5 lg:h-5 text-amber-600 shrink-0" />
                  <span>11-Gauge USA Structural Steel Tubing</span>
                </div>
                <div className="flex items-center gap-2">
                  <Cpu className="w-4 h-4 lg:w-5 lg:h-5 text-amber-600 shrink-0" />
                  <span>Zero-Friction Sealed Bearing Axles</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 lg:w-5 lg:h-5 text-amber-600 shrink-0" />
          ...
          {groupedCategories.map((group) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 border-b border-amber-200/80 pb-3">
                <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-700 shrink-0">
                  {group.icon}
                </div>
                <div>
                  <span className="text-amber-600 text-[10px] lg:text-xs font-mono uppercase tracking-[0.2em] font-bold block">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-7 font-sans">
                  ...
                  >
                    <div>
                      ...
                      <div className="relative aspect-video overflow-hidden bg-stone-100 border-b border-amber-200/80">
                        ...
                        <span className="absolute top-3 left-3 bg-white/90 border border-amber-200 text-amber-800 text-[9px] lg:text-[11px] font-mono uppercase tracking-widest px-2.5 py-1 lg:px-3 lg:py-1.5 rounded-full backdrop-blur-md font-bold shadow-sm">
                          {item.categoryLabel}
                        </span>

                        <button
                          onClick={() => setInspectItem(item)}
                          className="absolute bottom-3 right-3 bg-amber-600 text-white p-2 lg:p-2.5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-md hover:bg-amber-700 cursor-pointer"
                      ...
                      <div className="p-5 lg:p-6 space-y-2.5">
                        <h3 className="text-lg lg:text-xl font-bold text-stone-900 group-hover:text-amber-700 transition-colors font-serif">
                          {item.title}
                        </h3>
                        
                        <span className="block text-[11px] lg:text-xs font-mono text-amber-700 font-bold">
                          {item.subtitle}
                        </span>

                        <p className="text-xs lg:text-sm text-stone-600 font-normal leading-relaxed line-clamp-3">
                          {item.description}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="px-5 lg:px-6 pb-5 lg:pb-6 pt-2 border-t border-amber-100 mt-auto">
                      <div className="flex items-center justify-between text-[10px] lg:text-xs font-mono text-stone-500 mb-3">
                      ...
                      <button
                        onClick={() => setInspectItem(item)}
                        className="w-full text-center border border-amber-200 bg-white/80 hover:bg-gradient-to-r hover:from-amber-600 hover:to-yellow-600 hover:text-white hover:border-amber-600 text-amber-700 text-xs lg:text-sm font-bold uppercase tracking-wider py-2.5 lg:py-3 rounded-xl transition-all duration-300 cursor-pointer shadow-sm"
                      >
        ...
        {inspectItem && (
          <div className="fixed inset-0 z-50 bg-stone-950/50 backdrop-blur-md flex items-center justify-center p-4 font-sans">
            <div className="bg-white/95 backdrop-blur-sm border border-amber-200/80 rounded-3xl max-w-2xl lg:max-w-3xl w-full p-6 lg:p-8 space-y-4 relative shadow-xl max-h-[90vh] overflow-y-auto">
              ...
              {/* Biomechanics Box */}
              <div className="bg-amber-50/60 border border-amber-200/80 rounded-2xl p-3.5 lg:p-4 space-y-1">
                <span className="text-[10px] lg:text-xs font-mono uppercase tracking-widest text-amber-800 font-bold block">
                  Biomechanical Optimization
                </span>
                <p className="text-xs lg:text-sm text-stone-700 font-normal leading-relaxed">
              ...
              <div className="space-y-2">
                <h4 className="text-xs lg:text-sm font-mono uppercase tracking-widest text-stone-500 font-bold">
                  Technical Specifications
                </h4>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 lg:gap-3 text-xs lg:text-sm font-mono">
                  <div className="bg-stone-50/80 p-2.5 lg:p-3 rounded-xl border border-amber-200/80">
                    <span className="text-stone-400 block text-[9px] lg:text-[11px] uppercase">Steel / Construction</span>
                    <span className="text-stone-900 font-semibold">{inspectItem.specs.steelGauge}</span>
                  </div>

                  <div className="bg-stone-50/80 p-2.5 lg:p-3 rounded-xl border border-amber-200/80">
                    <span className="text-stone-400 block text-[9px] lg:text-[11px] uppercase">Maximum Rated Load</span>
                    <span className="text-amber-700 font-semibold">{inspectItem.specs.maxLoad}</span>
                  </div>

                  <div className="bg-stone-50/80 p-2.5 lg:p-3 rounded-xl border border-amber-200/80">
                    <span className="text-stone-400 block text-[9px] lg:text-[11px] uppercase">Adjustability</span>
                    <span className="text-stone-900 font-semibold">{inspectItem.specs.adjustability}</span>
                  </div>

                  <div className="bg-stone-50/80 p-2.5 lg:p-3 rounded-xl border border-amber-200/80">
                    <span className="text-stone-400 block text-[9px] lg:text-[11px] uppercase">Coating & Finish</span>
              ...
              <div className="pt-1">
                <button
                  onClick={() => setInspectItem(null)}
                  className="w-full bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-700 hover:to-yellow-700 text-white font-bold text-xs lg:text-sm uppercase tracking-[0.15em] py-3.5 lg:py-4 rounded-xl transition-colors shadow-md shadow-amber-600/25 cursor-pointer"
                >
                  Close Inspection
                </button>
              </div>
