...
export default function Workouts() {
  ...
  // 10 workout cards bana rahe hain (workout1, workout2, workout1, workout2, ...)
  const TOTAL_VIDEOS = 10;
  const extendedHomeWorkouts = Array.from({ length: TOTAL_VIDEOS }, (_, i) => {
    const base = homeWorkouts[i % homeWorkouts.length];
    return {
      ...base,
      id: `${base.id}-${i}`,
      _videoIndex: i, // decides workout1 vs workout2
    };
  });
  ...
  // Chota reusable "View" toggle button
  const ViewToggleButton = ({ isOpen, onClick }) => (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 text-[11px] sm:text-xs font-bold uppercase tracking-wider px-4 py-2.5 rounded-xl transition-all duration-300 border shadow-sm shrink-0 ${
        isOpen
          ...
          <section className="space-y-6">
            <div className="border-b border-amber-200/80 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <ViewToggleButton isOpen={gymOpen} onClick={() => setGymOpen((prev) => !prev)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {visibleHomeWorkouts.map((workout) => (
                ...
                >
                  ...
                  <VideoPlayer
                    videoUrl={localVideos[workout._videoIndex % localVideos.length]}
            ...
            {extendedHomeWorkouts.length > 4 && (
              <div className="flex justify-center pt-2">
                <button
                  ...
                  className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider px-6 py-3 rounded-xl transition-all duration-300 border shadow-sm ${
                    showAllVideos
                      ? 'bg-gradient-to-r from-amber-600 to-yellow-600 text-white border-amber-600 shadow-amber-600/20'
                      : 'bg-white/90 text-stone-600 border-amber-200 hover:text-amber-700 hover:border-amber-400 hover:bg-amber-50/50'
                  }`}
                >
                  ...
                  <span>{showAllVideos ? 'Show Less' : `View All Videos (${extendedHomeWorkouts.length})`}</span>
          ...
          <section className="space-y-6 pt-4">
            <div className="border-b border-amber-200/80 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <ViewToggleButton isOpen={strictOpen} onClick={() => setStrictOpen((prev) => !prev)} />
              </div>
            </div>
